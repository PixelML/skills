...
test -f .agent-skills/post-to-wechat/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/agent-skills/post-to-wechat/EXTEND.md" && echo "xdg"
test -f "$HOME/.agent-skills/post-to-wechat/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .agent-skills/post-to-wechat/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/agent-skills/post-to-wechat/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.agent-skills/post-to-wechat/EXTEND.md") { "user" }
```

┌────────────────────────────────────────────────────────┬───────────────────┐
│                          Path                          │     Location      │
├────────────────────────────────────────────────────────┼───────────────────┤
│ .agent-skills/post-to-wechat/EXTEND.md           │ Project directory │
├────────────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.agent-skills/post-to-wechat/EXTEND.md     │ User home         │
└────────────────────────────────────────────────────────┴───────────────────┘
...
3. `.agent-skills/.env` with prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `~/.agent-skills/.env` with prefixed key
...
| Profile dir | Shared profile at `agent-skills/chrome-profile` (see CLAUDE.md Chrome Profile section) |
...
| API credentials | Follow guided setup in Step 2, or manually set in `.agent-skills/.env` |
...
```bash
# macOS, Linux, WSL, Git Bash
test -f .agent-skills/.env && grep -q "WECHAT_APP_ID" .agent-skills/.env && echo "project"
test -f "$HOME/.agent-skills/.env" && grep -q "WECHAT_APP_ID" "$HOME/.agent-skills/.env" && echo "user"
```

```powershell
# PowerShell (Windows)
if ((Test-Path .agent-skills/.env) -and (Select-String -Quiet -Pattern "WECHAT_APP_ID" .agent-skills/.env)) { "project" }
if ((Test-Path "$HOME/.agent-skills/.env") -and (Select-String -Quiet -Pattern "WECHAT_APP_ID" "$HOME/.agent-skills/.env")) { "user" }
```
...
Where to save?
A) Project-level: .agent-skills/.env (this project only)
B) User-level: ~/.agent-skills/.env (all projects)
```
...
- Guided setup in Step 2, or manually set in `.agent-skills/.env`
