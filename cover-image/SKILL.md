...
test -f .agent-skills/cover-image/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/agent-skills/cover-image/EXTEND.md" && echo "xdg"
test -f "$HOME/.agent-skills/cover-image/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .agent-skills/cover-image/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/agent-skills/cover-image/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.agent-skills/cover-image/EXTEND.md") { "user" }
```
