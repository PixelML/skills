...
test -f .agent-skills/xhs-images/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/agent-skills/xhs-images/EXTEND.md" && echo "xdg"
test -f "$HOME/.agent-skills/xhs-images/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .agent-skills/xhs-images/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/agent-skills/xhs-images/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.agent-skills/xhs-images/EXTEND.md") { "user" }
```

┌────────────────────────────────────────────────────┬───────────────────┐
│                        Path                        │     Location      │
├────────────────────────────────────────────────────┼───────────────────┤
│ .agent-skills/xhs-images/EXTEND.md                 │ Project directory │
├────────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.agent-skills/xhs-images/EXTEND.md           │ User home         │
└────────────────────────────────────────────────────┴───────────────────┘
...
**Image Generation**: Uses the `image-gen` skill. In AgenticFlow mode, images are generated via `agenticflow workflow run`. See the `image-gen` skill for setup. In local mode, uses the TypeScript scripts directly.
...
**Style x Layout Matrix** (✓✓ = highly recommended, ✓ = works well):
