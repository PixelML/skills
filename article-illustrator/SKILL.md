...
**Image Generation**: Uses the `image-gen` skill. In AgenticFlow mode, images are generated via `agenticflow workflow run`. See the `image-gen` skill for setup.
...
test -f .agent-skills/article-illustrator/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/agent-skills/article-illustrator/EXTEND.md" && echo "xdg"
test -f "$HOME/.agent-skills/article-illustrator/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .agent-skills/article-illustrator/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/agent-skills/article-illustrator/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.agent-skills/article-illustrator/EXTEND.md") { "user" }
```
...
**Execution strategy**: When multiple illustrations have saved prompt files and the task is now plain generation, prefer `image-gen` batch mode (`build-batch.ts` → `--batchfile`) over spawning subagents. Use subagents only when each image still needs separate prompt iteration or creative exploration.
